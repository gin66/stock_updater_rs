...
impl Game for Tour {
    type Input = coffee::input::KeyboardAndMouse;
    type LoadingScreen = ();
    const TICKS_PER_SECOND: u16 = 30;
    const DEBUG_KEY: Option<KeyCode> = Some(KeyCode::D);
    ...
    fn draw(&mut self, frame: &mut Frame, _timer: &Timer) {
        ...
        match self.mode {
            ...
            ModeOption::Stroke => {
                mesh.stroke(shape, self.color, 2);
            }
        }
        ...
        image.draw(quad, &mut frame.as_target());
    }
}
    ...
    fn react(&mut self, event: Message) {
        match event {
            ...
            Message::StepMessage(step_msg) => {
                self.steps.update(step_msg, &mut self.buf);
            }
        }
    }
...
impl Steps {
    ...
    fn update(&mut self, msg: StepMessage, buf: &mut image::DynamicImage) {
        self.steps[self.current].update(msg, buf);
    }
...
impl<'a> Step {
    fn update(&mut self, msg: StepMessage, buf: &'a mut image::DynamicImage) {
        match msg {
